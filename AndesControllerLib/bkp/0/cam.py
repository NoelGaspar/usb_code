...
#
# Use this module to interact with SciCam
#
...
class Camera:
    ...
    with usbEasy.Device(vid = self._vid, pid = self._pid, context = self.context) as dev:
      ...
      for line in bytecode_lines:
        ...
        if(response == _success_cache):
          self.log.info('Recieved SUCCESS', 10);              
        else:
          self.log.error('Recieved: (len ' + str(len(response)) + ') ' + str(list(response[:4])), 1);
  ...
  def get_temperatures(self):
    ...
    to_return = {};
    if(len(result) != 1):
      self.log.error('Recieved different number of temperature reads ( ' + str(len(result)) + ' ) than expected (1).');
    else:
      self.log.info('Recieved: (len ' + str(len(result)) + ')', 10);
    ...
    with usbEasy.Device(vid = self._vid, pid = self._pid, context = self.context) as dev:
      ...
      for line in bytecode_lines:
        ...
        if(response == _success_cache):
          ...
          self.log.error('Recieved: (len ' + str(len(response)) + ') ' + str(response), 1);
  ...
  def take_picture(self):
    ...
    if len(raw_data) < struct.calcsize(fmt):
      raise RuntimeError( 'The amount of recieved bytes (' + str(len(raw_data)) + ') ' + \
