...
class ByteCode:
	...
	#
	# @param self An instance of ByteCode.
	# @param stop_cleaning_mode_dir (int) Address of the stop cleaning mode.
	# @param get_image_mode_dir (int) Address of the get image mode.
	...
	#
	# @param self An instance of ByteCode.
	# @param channel (bool) Channel to read (True:1, False:3).
	#
	# @returns A list of codes ([int]).
	def get_pixels_channel(self, channel=True):
		header_submodule   = self._acquisition_sequencer_index;  #0;
		header_instruction = self._acquisition_sequencer_getpxlsch1_inst if channel else self._acquisition_sequencer_getpxlsch3_inst
		result =  [ \
					self._init,
					self._acquisition_mod_index, 
					self.int_to_byte_list(1), 
	...
	#
	# @returns A list of codes ([int]).
	def get_data_channel(self, channel=True, samples=0):
		header_submodule   = self._acquisition_sequencer_index;  #0;
		header_instruction = self._acquisition_sequencer_getdatach1_inst if channel else self._acquisition_sequencer_getdatach3_inst
		result =  [ \
					self._init,
					self._acquisition_mod_index, 
					self.int_to_byte_list(2), 
