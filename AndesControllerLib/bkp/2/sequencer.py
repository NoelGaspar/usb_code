		...
		for mode in self.modes:
			...
			if(mode.is_nested()):
				try:
					...
					if(parent.is_nested()):
						self.log.warning('Double nested modes detected: ' + str(mode.name) + ' in ' + str(parent.name) + ' in ' + str(parent.parent_mode_name) + '.');
				except:
					error = True;
...
class Mode:
	...
	def get_code(self, address_cache):
		...
		code = 0;									# 64 bits total (2 x 32 bit words)
		code = code | nested_loops;					# 16 bits
		code = code | (parent_address << 16);		# 10 bits
		code = code | (next_address << 26);			# 10 bits
		code = code | (is_nested << 36);			#  1 bit
		code = code | (self.n_loops << 37);			# 16 bits
		code = code | (len(self.states) << 53);		# 10 bits
		code = code | (1 << 63);					#  1 bit
...
class State:
	...
	def from_bits(cls, data_bits, hold_time):
		...
		if(len(bits) < 32):
			log.warning('There are less bits than the expected in a state (' + str(len(bits) ) + '/32), will fill MSBs with 0s.');
