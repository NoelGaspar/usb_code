...
## @package ccd
#
# Generation of Andes Controller programs for specific CCD's.
#
...
class _CCD:
	...
	# @param self An instance of _CCD
	#  
	# @returns A sequencer.Program for Andes Controller sequencer programming.
	...
	#
	# @param self An instance of _CCD
	# @param bytecode_formatter A binary.ByteCode to format writing instructions.
	#  
	...
	def get_configuration_bytecode(self, bytecode_formatter):
		...
		# Initialize Power
		# bytecode_lines.append(formatter.configurator_power_on('bias_analog',    False));
		# bytecode_lines.append(formatter.configurator_power_on('clocks_analog',  False));
		# bytecode_lines.append(formatter.configurator_power_on('video',          False));
		# bytecode_lines.append(formatter.configurator_power_on('clocks_digital', True));
		# bytecode_lines.append(formatter.configurator_power_on('bias_digital',   True));
		bytecode_lines.append(formatter.configurator_power_on(True));
		bytecode_lines.append(formatter.dacs_pwr_reset());

		# Initialize digital DACs
		for word in word_config_codes['BiasClocksConfig']:
			bytecode_lines.append(formatter.configurator_spi_bias_clocks(word.dev, word.pol, word.nbits, word.code));

		for word in word_config_codes['BiasClocksVoltage']:
			bytecode_lines.append(formatter.configurator_spi_bias_clocks(word.dev, word.pol, word.nbits, (word.address<<16) + word.voltage));

		# Initialize Sequencer
		for ii in range(len(self.configured_program.codes)):
			seq_line = self.configured_program.codes[ii];
			bytecode_lines.append(formatter.write_sequencer_memory(ii, seq_line));

		bytecode_lines.append(formatter.enable_sequencer());

		# Initialize analog DACs
		# bytecode_lines.append(formatter.configurator_power_on('video', True));

		for word in word_config_codes['VideoConfig']:
			bytecode_lines.append(formatter.configurator_spi_video(word.dev, word.pol, word.nbits, word.code));

		for word in word_config_codes['VideoVoltage']:
			bytecode_lines.append(formatter.configurator_spi_video(word.dev, word.pol, word.nbits, (word.address<<16) + word.voltage));

		for word in word_config_codes['VideoAdcConfig']:
			bytecode_lines.append(formatter.configurator_spi_video(word.dev, word.pol, word.nbits, word.code));

		# bytecode_lines.append(formatter.configurator_power_on('bias_analog',  True));
		# bytecode_lines.append(formatter.configurator_power_on('clocks_analog',True));

		return bytecode_lines;

	## Gets the name of the sequencer mode in which to stop the cleaning routine.
	#
	# @param self An instance of _CCD
	#  
	# @returns The mode name (str)
	def get_stop_cleaning_mode_name(self):
		raise NotImplementedError('Override _CCDs get_stop_cleaning_mode_name!');

	## Gets the name of the sequencer mode in which to start taking an image.
	#
	# @param self An instance of _CCD
	#  
	# @returns The mode name (str)
	def get_get_image_mode_name(self):
		raise NotImplementedError('Override _CCDs get_get_image_mode_name!');
...
## This is a specialization of _CCD for the ccd CCD_230_42 from e2V.
# @see _CCD
class CCD_230_42(_CCD):
	...
	_default_clock_params = {
		# External clocks
		'CLK1A':{'dev':0, 'pol':0, 'nbits':24, 'add_bot': 0, 'add_top': 1, 'volt_bot':0, 'volt_top':0,  'clockId': 0},
		'CLK2A':{'dev':0, 'pol':0, 'nbits':24, 'add_bot': 3, 'add_top': 2, 'volt_bot':0, 'volt_top':0,  'clockId': 1},
		'CLK3A':{'dev':0, 'pol':0, 'nbits':24, 'add_bot': 4, 'add_top': 5, 'volt_bot':0, 'volt_top':0,  'clockId': 2},
		'CLK4A':{'dev':0, 'pol':0, 'nbits':24, 'add_bot': 7, 'add_top': 6, 'volt_bot':0, 'volt_top':0,  'clockId': 3},
		'CLK5A':{'dev':0, 'pol':0, 'nbits':24, 'add_bot': 8, 'add_top': 9, 'volt_bot':0, 'volt_top':0,  'clockId': 4},
		'CLK6A':{'dev':0, 'pol':0, 'nbits':24, 'add_bot':11, 'add_top':10, 'volt_bot':0, 'volt_top':0,  'clockId': 5},
		'CLK7A':{'dev':0, 'pol':0, 'nbits':24, 'add_bot':12, 'add_top':13, 'volt_bot':0, 'volt_top':0,  'clockId': 6},
		'CLK8A':{'dev':0, 'pol':0, 'nbits':24, 'add_bot':15, 'add_top':14, 'volt_bot':0, 'volt_top':0,  'clockId': 7},

		'CLK1B':{'dev':1, 'pol':0, 'nbits':24, 'add_bot': 0, 'add_top': 1, 'volt_bot':0, 'volt_top':0,  'clockId': 8},
		'CLK2B':{'dev':1, 'pol':0, 'nbits':24, 'add_bot': 3, 'add_top': 2, 'volt_bot':0, 'volt_top':0,  'clockId': 9},
		'CLK3B':{'dev':1, 'pol':0, 'nbits':24, 'add_bot': 4, 'add_top': 5, 'volt_bot':0, 'volt_top':0,  'clockId':10},
		'CLK4B':{'dev':1, 'pol':0, 'nbits':24, 'add_bot': 7, 'add_top': 6, 'volt_bot':0, 'volt_top':0,  'clockId':11},
		'CLK5B':{'dev':1, 'pol':0, 'nbits':24, 'add_bot': 8, 'add_top': 9, 'volt_bot':0, 'volt_top':0,  'clockId':12},
		'CLK6B':{'dev':1, 'pol':0, 'nbits':24, 'add_bot':11, 'add_top':10, 'volt_bot':0, 'volt_top':0,  'clockId':13},
		'CLK7B':{'dev':1, 'pol':0, 'nbits':24, 'add_bot':12, 'add_top':13, 'volt_bot':0, 'volt_top':0,  'clockId':14},
		'CLK8B':{'dev':1, 'pol':0, 'nbits':24, 'add_bot':15, 'add_top':14, 'volt_bot':0, 'volt_top':0,  'clockId':15},

		'CLK1C':{'dev':2, 'pol':0, 'nbits':24, 'add_bot': 0, 'add_top': 1, 'volt_bot':0, 'volt_top':0,  'clockId':16},
		'CLK2C':{'dev':2, 'pol':0, 'nbits':24, 'add_bot': 3, 'add_top': 2, 'volt_bot':0, 'volt_top':0,  'clockId':17},
		'CLK3C':{'dev':2, 'pol':0, 'nbits':24, 'add_bot': 4, 'add_top': 5, 'volt_bot':0, 'volt_top':0,  'clockId':18},
		'CLK4C':{'dev':2, 'pol':0, 'nbits':24, 'add_bot': 7, 'add_top': 6, 'volt_bot':0, 'volt_top':0,  'clockId':19},
		'CLK5C':{'dev':2, 'pol':0, 'nbits':24, 'add_bot': 8, 'add_top': 9, 'volt_bot':0, 'volt_top':0,  'clockId':20},
		'CLK6C':{'dev':2, 'pol':0, 'nbits':24, 'add_bot':11, 'add_top':10, 'volt_bot':0, 'volt_top':0,  'clockId':21},
		'CLK7C':{'dev':2, 'pol':0, 'nbits':24, 'add_bot':12, 'add_top':13, 'volt_bot':0, 'volt_top':0,  'clockId':22},
		'CLK8C':{'dev':2, 'pol':0, 'nbits':24, 'add_bot':15, 'add_top':14, 'volt_bot':0, 'volt_top':0,  'clockId':23},

		'CLK1D':{'dev':3, 'pol':0, 'nbits':24, 'add_bot': 0, 'add_top': 1, 'volt_bot':0, 'volt_top':0,  'clockId':24},
		'CLK2D':{'dev':3, 'pol':0, 'nbits':24, 'add_bot': 3, 'add_top': 2, 'volt_bot':0, 'volt_top':0,  'clockId':25},
		'CLK3D':{'dev':3, 'pol':0, 'nbits':24, 'add_bot': 4, 'add_top': 5, 'volt_bot':0, 'volt_top':0,  'clockId':26},
		'CLK4D':{'dev':3, 'pol':0, 'nbits':24, 'add_bot': 7, 'add_top': 6, 'volt_bot':0, 'volt_top':0,  'clockId':27},
		'CLK5D':{'dev':3, 'pol':0, 'nbits':24, 'add_bot': 8, 'add_top': 9, 'volt_bot':0, 'volt_top':0,  'clockId':28},
		'CLK6D':{'dev':3, 'pol':0, 'nbits':24, 'add_bot':11, 'add_top':10, 'volt_bot':0, 'volt_top':0,  'clockId':29},
		'CLK7D':{'dev':3, 'pol':0, 'nbits':24, 'add_bot':12, 'add_top':13, 'volt_bot':0, 'volt_top':0,  'clockId':30},
		'CLK8D':{'dev':3, 'pol':0, 'nbits':24, 'add_bot':15, 'add_top':14, 'volt_bot':0, 'volt_top':0,  'clockId':31},

		# Internal clocks
		'CLKI1':{'dev':-1, 'pol':0, 'nbits':0,  'add_bot': 0, 'add_top': 0, 'volt_bot':0, 'volt_top':0,  'clockId':32},
		'CLKI2':{'dev':-1, 'pol':0, 'nbits':0,  'add_bot': 0, 'add_top': 0, 'volt_bot':0, 'volt_top':0,  'clockId':33},
		'CLKI3':{'dev':-1, 'pol':0, 'nbits':0,  'add_bot': 0, 'add_top': 0, 'volt_bot':0, 'volt_top':0,  'clockId':34},
		'CLKI4':{'dev':-1, 'pol':0, 'nbits':0,  'add_bot': 0, 'add_top': 0, 'volt_bot':0, 'volt_top':0,  'clockId':35},
		'CLKI5':{'dev':-1, 'pol':0, 'nbits':0,  'add_bot': 0, 'add_top': 0, 'volt_bot':0, 'volt_top':0,  'clockId':36},
		'CLKI6':{'dev':-1, 'pol':0, 'nbits':0,  'add_bot': 0, 'add_top': 0, 'volt_bot':0, 'volt_top':0,  'clockId':37},
		'CLKI7':{'dev':-1, 'pol':0, 'nbits':0,  'add_bot': 0, 'add_top': 0, 'volt_bot':0, 'volt_top':0,  'clockId':38},
		'CLKI8':{'dev':-1, 'pol':0, 'nbits':0,  'add_bot': 0, 'add_top': 0, 'volt_bot':0, 'volt_top':0,  'clockId':39},
	}

	## Default value for biases
	# bias_vect order name bus device polarity nbits address voltage
	_default_bias_params = {
		'BHV1A':{'dev':4, 'pol':0, 'nbits':24, 'address': 0, 'voltage':0, 'biasId': 40, 'voltType': 'high'},
		'BHV2A':{'dev':4, 'pol':0, 'nbits':24, 'address': 1, 'voltage':0, 'biasId': 41, 'voltType': 'high'},
		'BHV3A':{'dev':4, 'pol':0, 'nbits':24, 'address': 2, 'voltage':0, 'biasId': 42, 'voltType': 'high'},
		'BHV4A':{'dev':4, 'pol':0, 'nbits':24, 'address': 3, 'voltage':0, 'biasId': 43, 'voltType': 'high'},
		'BHV5A':{'dev':4, 'pol':0, 'nbits':24, 'address': 4, 'voltage':0, 'biasId': 44, 'voltType': 'high'},
		'BHV6A':{'dev':4, 'pol':0, 'nbits':24, 'address': 5, 'voltage':0, 'biasId': 45, 'voltType': 'high'},
		'BHV7A':{'dev':4, 'pol':0, 'nbits':24, 'address': 6, 'voltage':0, 'biasId': 46, 'voltType': 'high'},
		'BHV8A':{'dev':4, 'pol':0, 'nbits':24, 'address': 7, 'voltage':0, 'biasId': 47, 'voltType': 'high'},
		'BLV1A':{'dev':4, 'pol':0, 'nbits':24, 'address': 8, 'voltage':0, 'biasId': 48, 'voltType': 'low' },
		'BLV2A':{'dev':4, 'pol':0, 'nbits':24, 'address': 9, 'voltage':0, 'biasId': 49, 'voltType': 'low' },
		'BLV3A':{'dev':4, 'pol':0, 'nbits':24, 'address':10, 'voltage':0, 'biasId': 50, 'voltType': 'low' },
		'BLV4A':{'dev':4, 'pol':0, 'nbits':24, 'address':11, 'voltage':0, 'biasId': 51, 'voltType': 'low' },
		'BLV5A':{'dev':4, 'pol':0, 'nbits':24, 'address':12, 'voltage':0, 'biasId': 52, 'voltType': 'low' },
		'BLV6A':{'dev':4, 'pol':0, 'nbits':24, 'address':13, 'voltage':0, 'biasId': 53, 'voltType': 'low' },
		'BLV7A':{'dev':4, 'pol':0, 'nbits':24, 'address':14, 'voltage':0, 'biasId': 54, 'voltType': 'low' },
		'BLV8A':{'dev':4, 'pol':0, 'nbits':24, 'address':15, 'voltage':0, 'biasId': 55, 'voltType': 'low' },

	    'BHV1B':{'dev':5, 'pol':0, 'nbits':24, 'address': 0, 'voltage':0, 'biasId':56, 'voltType': 'high'},
	    'BHV2B':{'dev':5, 'pol':0, 'nbits':24, 'address': 1, 'voltage':0, 'biasId':57, 'voltType': 'high'},
	    'BHV3B':{'dev':5, 'pol':0, 'nbits':24, 'address': 2, 'voltage':0, 'biasId':58, 'voltType': 'high'},
	    'BHV4B':{'dev':5, 'pol':0, 'nbits':24, 'address': 3, 'voltage':0, 'biasId':59, 'voltType': 'high'},
	    'BHV5B':{'dev':5, 'pol':0, 'nbits':24, 'address': 4, 'voltage':0, 'biasId':60, 'voltType': 'high'},
	    'BHV6B':{'dev':5, 'pol':0, 'nbits':24, 'address': 5, 'voltage':0, 'biasId':61, 'voltType': 'high'},
	    'BHV7B':{'dev':5, 'pol':0, 'nbits':24, 'address': 6, 'voltage':0, 'biasId':62, 'voltType': 'high'},
	    'BHV8B':{'dev':5, 'pol':0, 'nbits':24, 'address': 7, 'voltage':0, 'biasId':63, 'voltType': 'high'},
	   	'BLV1B':{'dev':5, 'pol':0, 'nbits':24, 'address': 8, 'voltage':0, 'biasId':64, 'voltType': 'low' },
	   	'BLV2B':{'dev':5, 'pol':0, 'nbits':24, 'address': 9, 'voltage':0, 'biasId':65, 'voltType': 'low' },
	   	'BLV3B':{'dev':5, 'pol':0, 'nbits':24, 'address':10, 'voltage':0, 'biasId':66, 'voltType': 'low' },
	   	'BLV4B':{'dev':5, 'pol':0, 'nbits':24, 'address':11, 'voltage':0, 'biasId':67, 'voltType': 'low' },
	   	'BLV5B':{'dev':5, 'pol':0, 'nbits':24, 'address':12, 'voltage':0, 'biasId':68, 'voltType': 'low' },
	   	'BLV6B':{'dev':5, 'pol':0, 'nbits':24, 'address':13, 'voltage':0, 'biasId':69, 'voltType': 'low' },
	  	'BLV7B':{'dev':5, 'pol':0, 'nbits':24, 'address':14, 'voltage':0, 'biasId':70, 'voltType': 'low' },
	   	'BLV8B':{'dev':5, 'pol':0, 'nbits':24, 'address':15, 'voltage':0, 'biasId':71, 'voltType': 'low' },
	
}
	## Default value for video_offset
	# bias_vect order name bus device polarity nbits address voltage
	_default_video_params = {
		'OFFCH1':{'dev':2, 'pol':0, 'nbits':24, 'address': 0, 'voltage':0, 'videoId': 0},
		'OFFCH2':{'dev':2, 'pol':0, 'nbits':24, 'address': 1, 'voltage':0, 'videoId': 1},
		'OFFCH3':{'dev':3, 'pol':0, 'nbits':24, 'address': 0, 'voltage':0, 'videoId': 2},
		'OFFCH4':{'dev':3, 'pol':0, 'nbits':24, 'address': 1, 'voltage':0, 'videoId': 3},
	}

	## Default value for video_ADC
	# bias_vect order name bus device polarity nbits address voltage
	_default_video_ADC_params = {
		'ADCCH1':{'dev':0, 'pol':1, 'nbits':16, 'config':(0x00000080, 0x00000100, 0x00000241, 0x00000355, 0x00000455)},
		'ADCCH2':{'dev':1, 'pol':1, 'nbits':16, 'config':(0x00000080, 0x00000100, 0x00000241, 0x00000355, 0x00000455)},
	}
	...
	# 4 Outputs
		...
		'PC4U' : _default_clock_params['CLK4C']['clockId'],  # SEQ04  # C4 D4 Parallel Clock Up
		'SC1L' : _default_clock_params['CLK1B']['clockId'],  # SEQ07  # E2 H2 Serial Clock Left
		'SC2L' : _default_clock_params['CLK2B']['clockId'],  # SEQ08  # E1 H1 Serial Clock Left
		'SC1R' : _default_clock_params['CLK8A']['clockId'],  # SEQ13  # F2 G2 Serial Clock Right
		'SC2R' : _default_clock_params['CLK7A']['clockId'],  # SEQ12  # F1 G1 Serial Clock Right
		'SCO'  : _default_clock_params['CLK6A']['clockId'],  # SEQ10  # E F G H Serial Clock Output
		'TGD'  : _default_clock_params['CLK6B']['clockId'],  # SEQ21  # A B Transfer Gate, Must be equal to PC1. Down
		'TGU'  : _default_clock_params['CLK7B']['clockId'],  # SEQ00  # C D Transfer Gate, Must be equal to PC1. Up
		'ORL'  : _default_clock_params['CLK4B']['clockId'],  # SEQ23  # E H Reset Left
		'ORR'  : _default_clock_params['CLK4A']['clockId'],  # SEQ02  # F G Reset Right
		'SWOL' : _default_clock_params['CLK3B']['clockId'],  # SEQ06  # E H Summing Well Left
		'SWOR' : _default_clock_params['CLK5A']['clockId'],  # SEQ14  # F G Summing Well Right
		'DGD'  : _default_clock_params['CLK1A']['clockId'],  # SEQ09  # A B Dump Gate Down
		'DGU'  : _default_clock_params['CLK2A']['clockId'],  # SEQ11  # C D Dump Gate Up
		'PIXT' : _default_clock_params['CLK3A']['clockId'],  # SEQ30  # Pixel Time
		'CRST' : _default_clock_params['CLK5B']['clockId'],  # SEQ29  # DC Level Reset, Video Capacitor
	};

	## Default value for clock_order
	_default_clock_order = ['PC1D','PC2D','PC3D','PC4D','PC1U','PC2U','PC3U','PC4U','SC1L','SC2L','SC1R','SC2R',\
							'SCO' ,'TGD' ,'TGU' ,'ORL' ,'ORR' ,'SWOL','SWOR','DGD' ,'DGU' ,'PIXT','CRST'];

	## Set default clock_voltages
	_default_clock_voltages = {
		...
		'DGD'  : {'volt_top':  +3.0, 'volt_bot': -9.0},		# 3.0
		'DGU'  : {'volt_top':  +3.0, 'volt_bot': -9.0},
	}

	## Default value for bias_pins, mount names
	_default_bias_pins = {
		'RDL'  : _default_bias_params['BLV4A']['biasId'],         # RD-E, RD-H;
		'RDR'  : _default_bias_params['BLV6B']['biasId'],         # RD-F, RD-G;
		'ODL'  : _default_bias_params['BHV3A']['biasId'],         # OD-E, OD-H;
		'ODR'  : _default_bias_params['BHV6B']['biasId'],         # OD-F, OD-G;
		'OGL'  : _default_bias_params['BLV3A']['biasId'],         # OG-E, OG-H;
		'OGR'  : _default_bias_params['BLV7A']['biasId'],         # OG-F, OG-G;
		'DDD'  : _default_bias_params['BHV4A']['biasId'],         # DUMP DRAIN A
		'DDU'  : _default_bias_params['BHV5B']['biasId'],         # DUMP DRAIN D
...
#		'RDH'  : _default_bias_params['BLV1A']['biasId'],         #
	...
	## Default value for bias_voltages
...
#		'RDH'  : {'voltage':  +9.0},
	...
	## Default value for video_pins (video offset ids)
	_default_video_pins = {
	...
	## Set default video_voltages (video offset)
	...
	_default_n_cols = 2048;
	## Default value for n_rows
	_default_n_rows = 2064;
	...
	## Initializes a CDD_230_42. See _CCD
	#
	# @param self An instance of CDD_230_42
	# @param kwargs A dict containing the attributes of the object.
	#
	# @see _CCD
	def __init__(self, **kwargs):
		...
		self.add_default(kwargs, 'clock_order',     self._default_clock_order);
	...
	def _dac_bias_volt_to_code(self, voltage, voltType):
		voltCode = 0;
		if(voltType == 'high'):
			voltCode = int( ( ( ( voltage/6.0 ) / 5.0 ) * (2**14) ) + 0b1100000000000000 ); # {REG1, REG0} = 0b11 ; write to input  data register x1
		elif(voltType == 'low'):
	...
	def _set_dac_initial_voltages(self, voltages):
		for key in voltages:
			if(key in self.clock_pins.keys()):
				...
				raise ValueError('Label ' + str(key) + " doesn't exist !");
	...
	def _create_word_codes(self):
		...
		VideoConfigCode       = [];
		VideoAdcConfigCode    = [];
		BiasClocksVoltageWord = namedtuple('BiasClocksVoltageWord', 'dev pol nbits address voltage');
		BiasClocksConfigWord  = namedtuple('BiasClocksConfigWord',  'dev pol nbits code');
		VideoVoltageWord      = namedtuple('VideoVoltageWord',      'dev pol nbits address voltage');
		VideoConfigWord       = namedtuple('VideoConfigWord',       'dev pol nbits code');
		VideoAdcConfigWord    = namedtuple('VideoAdcConfigWord',    'dev pol nbits code');
		Devices = [];
		...
		for k in self.biases.keys():
			...
			BiasClocksVoltageCode.append(BiasClocksVoltageWord(dev, pol, nbits, addr, volt));
			tup = [dev];
			if(tup not in Devices):
				Devices.append(tup);
				BiasClocksConfigCode.append(BiasClocksConfigWord(dev, pol, nbits, 0x000C1D00));
		...
		for k in self.video_offset.keys():
			...
			VideoVoltageCode.append(VideoVoltageWord(dev, pol, nbits, addr, volt));
			if(VideoConfigWord(dev, pol, nbits, 0x00200000) not in VideoConfigCode):
				VideoConfigCode.append(VideoConfigWord(dev, pol, nbits, 0x00200000));
			if(VideoConfigWord(dev, pol, nbits, 0x00300000) not in VideoConfigCode):
				VideoConfigCode.append(VideoConfigWord(dev, pol, nbits, 0x00300000));
			if(VideoConfigWord(dev, pol, nbits, 0x00380001) not in VideoConfigCode):
				VideoConfigCode.append(VideoConfigWord(dev, pol, nbits, 0x00380001));
		...
		for ii in range(5):
			for k in self.video_ADC.keys():
				...
				conf  = self.video_ADC[k]['config'][ii];
				VideoAdcConfigCode.append(VideoAdcConfigWord(dev, pol, nbits, conf));
	...
	def _create_sequencer_program(self):
		...
		# Modes: cleaning_repeat, cleaning_end, exposing, init_sweep_out, end_sweep_out,
		#        parallel, init_binning, binning, end_binning, cleaning_init
